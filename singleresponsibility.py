...
"""Fråga:
Är det bättre att göra en lång definition av alla arguments. Eller flera olika, men alla behöver köra parsern först?"""
...
def check_port(target, port, timeout):
    ...
    try:
        ...
        # If a port is open, add the open port to the check_ports list
        if result == 0:
            ...
            try:
                ...
                if banner:
                    # Add open port to the check_ports list
                    check_ports = (f"Port {port} : Banner {banner}")
                    return check_ports
                else:
                    check_ports = (f"Port {port} : No banner received")
                    return check_ports
            # Socket timed out error
            except socket.timeout:
                check_ports = (f"Port {port} : No banner (timeout)")
                return check_ports
            # Catch other errors
            except Exception as e:
                check_ports = (f"Port {port} : Error reading banner {e}")
                return check_ports
    # DNS lookup failed error
    except socket.gaierror as e:
        check_ports = (f"{target} Hostname could not be resolved. {e}")
        return check_ports
    # Socket error
    except socket.error as e:
        check_ports = (f"{target} Could not connect to server. {e}")
        return check_ports
    # Close socket
    finally:
        s.close()
            ...
            for ips, start_port, max_port in zip(targets, start_ports, max_ports):
                ...
                # Set range of ports, including the max port
                    for port in range(start_port, max_port + 1):
                        port_result = check_port(target, port, timeout)
                        progress_bar.update(1)
                        if port_result:
                            f.write(f"{port_result}\n")
